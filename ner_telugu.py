...
Original file is located at
    https://colab.research.google.com/drive/1nX_QYLqak4EHijKl1mFznbSNfX6dMjAT
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ["B-NAME", "B-LOC", "O", "B-ORG", "B-MISC", "[CLS]", "[SEP]", "X"]
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer,mode):
    ...
    for (i, label) in enumerate(label_list,1):
        label_map[label] = i
    label_path = os.path.join(mid_out_dir, "label2id.pkl")
    with open(label_path,'wb') as w:
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("loss"):
        ...
        logits = tf.reshape(logits, [-1, max_seq_length, 9])
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        elif mod e == tf.estimator.ModeKeys.EVAL:
            
            def metric_fn(per_example_loss, label_ids, logits):
            # def metric_fn(label_ids, logits):
                predictions = tf.argmax(logits, axis=-1, output_type=tf.int32)
                precision = tf_metrics.precision(label_ids,predictions,9,[1,2,4,5],average="macro")
                recall = tf_metrics.recall(label_ids,predictions,9,[1,2,4,5],average="macro")
                f = tf_metrics.f1(label_ids,predictions,9,[1,2,4,5],average="macro")
                #
                return {
...
task_name = 'NER'
vocab_file = os.path.join(data_dir, "multi_cased_L-12_H-768_A-12", "vocab.txt")
bert_config_file = os.path.join(data_dir, "multi_cased_L-12_H-768_A-12", "bert_config.json")
output_dir = os.path.join(data_dir, "final_results")
mid_out_dir = os.path.join(data_dir, "mid_output")
init_checkpoint = os.path.join(data_dir, "multi_cased_L-12_H-768_A-12", "bert_model.ckpt")
do_train = False  
do_eval = True   
do_predict = False 
max_seq_length = 128
...
# INFO:tensorflow:  eval_f = 0.9147934
# INFO:tensorflow:  eval_precision = 0.91529953
# INFO:tensorflow:  eval_recall = 0.9144303
# INFO:tensorflow:  global_step = 653
# INFO:tensorflow:  loss = 13.282977

if do_predict:
    
    token_path = os.path.join(output_dir, "token_test.txt")
    label_path = os.path.join(mid_out_dir, "label2id.pkl")
    with open(label_path,'rb') as rf:

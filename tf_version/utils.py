...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["[PAD]", "B-NAME", "B-LOC", "O", "B-ORG", "B-MISC", "[CLS]", "[SEP]", "X"]
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, middle_output, mode):
    """
    :param middle_output:
    ...
    tokens = []
    labels = []
    for i, (word, label) in enumerate(zip(text_list, labellist)):
...
## Pre-processing Done
## Below are model creation and error calculation

def hidden2tag(hidden_layer, num_class):
    linear = tf.keras.layers.Dense(num_class, activation=None)
    return linear(hidden_layer)
...
def create_model(bert_config, is_training, input_ids, mask,
                 segment_ids, labels, num_labels, use_one_hot_embeddings, max_seq_length, crf):
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, max_seq_length, crf):
    def model_fn(features, labels, mode, params):
        ...
        print("HELLO:", tvars[0])
        print("INIT:", initialized_variable_names)
        print("CHECK:", tvars[0].name)
        for var in tvars:
            init_string = ""
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, logits, num_labels, mask):
                ...
                print("LABELs0:", num_labels, label_ids, predictions, mask)
                # cm = streaming_confusion_matrix(label_ids, predictions, num_labels - 1, weights=mask)
                cm = streaming_confusion_matrix(label_ids, predictions, num_labels, weights=mask)
                return {
                    "confusion_matrix": cm
                }
                #
